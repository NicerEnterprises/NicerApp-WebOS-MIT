...
export class na3D_fileBrowser {
    ...
    create(t) {
        ...
        na.m.waitForCondition ('na3D.js : items ready?', function() {
            return t.items.length > 2 && t.started4;
        }, function () {
...
//if (it.name.match(/becoming insane/i)) debugger;
            if (it.model) {
                if (p) {
                    ...
                    var p3 = jsem.math.xy.pointOnCircle_angleInDegrees (0, 0, 1000 + (it.level * 250), p2 );
                    p.p2 = p2;
                    p.p3 = p3;
                } else {
                    p3 = { x : 1, y : 1 };
                ...
                if (it.name.match(/\.mp3$/)) {
                    if (!t.showFiles) { /*delete it.model;*/} else {
                        ...
                        it.model.position.y = Math.round( (
                            ...
                            (p?
                                (p3.y * it.level * p.levelIdx * 1000)
                        //+(p.p3.y)
                                +((p.row-1) * (p.pos.yField * 750))
                            : 0)
                            + (p?p.p3.x:0)
                        ...
                        it.model.position.z = Math.round( (
                            ...
                            //+ (p?p.levelIdx* -3000:0)
                            + (it.level > min ? -1 * p.depthOffsetValue * it.depth * 500 : 0)
                        ) / divider);
                        debugger;
                    }
                    ...
                    it.model.position.x = Math.round( (
                        ...
                        //+ (p.rowOffsetValue * 300)
                        //(p.model.position.x ? p.model.position.x : 0)
                        (p.ld3?p.pos.xField* p.ld3.cubeSideLengthCount * 500:0)
                        //+ (it.ld3?it.pos.xField* it.ld3.cubeSideLengthCount * 500:0)
                        //+(p.levelIdx * 100)
                        +(p?p.depth*p.level*p.levelIdx*-500:0)
                        +(p.model.position.x ? p.model.position.x : 0)
                        + ((it.column-1) *  500)
