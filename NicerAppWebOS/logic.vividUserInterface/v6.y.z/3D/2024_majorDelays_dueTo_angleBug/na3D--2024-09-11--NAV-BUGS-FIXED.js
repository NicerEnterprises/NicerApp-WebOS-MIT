...
*/
        ...
        switch (t.controlsKey) {
            ...
            switch (t.controlsKey) {
                ...
                case 4:
                    ...
                    t.controls.movementSpeed = 2000; t.controls.rollSpeed = Math.PI * 2 / 45;
    ...
    create(t) {
        ...
        na.m.waitForCondition ('na3D.js : items ready?', function() {
            return t.items.length > 2 && t.winners;
        }, function () {
    ...
    animate(t, callAnimate, e) {
        ...
        if (timeLapsed>1 && t.controlsKey!==4 && t.evt3 && t.evt3.type=='pointerdown' && (t.evt3.button===0||t.evt3.button===2)) {
            t.controlsKey = 4;
            changed = true;
    ...
    animateOLD_AND_WITH_ANGLE_BUG (t, evt) {
        //requestAnimationFrame( function(evt) { t.animate (t,evt) });
        //if (t.mouse.x!==0 || t.mouse.y!==0) {
            ...
            /*
            var dbg = {
                    "t.controls.deltaX" : t.controls.deltaX,
                    "t.controls.deltaY" : t.controls.deltaY
                };
...
//                debugger;
            ...
            if (t.controls.enabled) {
                /* done elsewhere these days, in pointerup()
                 if (t.flyControls.enabled) {
                    if (t.debug) console.log ("animate() : setting t.controls.setLookAt()");
                ...
                if (t.debug) console.log ("animate() : calling t.controls.update()");
                //if (t.controls._isUserControllingTruck) debugger;
            ...
            //if (t.cameraOrigin && t.cameraOrigin.x && t.middle && t.middle.x) {
                if (t.middle && t.middle.x) {
                    if (!t.started3) {
                        ...
                        t.orbitControls.position0.y = t.middle.y;
                        t.controls._camera.position.y = t.middle.y;
                        t.camera.position.y = t.middle.y;
            ...
            if (t.flyControls.enabled && t.flyControls.movementSpeed > 0 && t.flyControls.movementSpeed < 1750)
            ...
            if (t.lookClock > 0 && delta2 > t.lookClock) {
                //debugger;
                if (t.debug) console.log ("t.flyControls.enabled, t.controls.disabled");
                t.flyControls.enabled = true;
                t.orbitControls.enabled = false;
                t.controlsEnabled = false;
            } else {
                if (t.debug) console.log ("t.flyControls.disabled, t.controls.enabled");
                t.flyControls.enabled = false;
                t.orbitControls.enabled = true;
            ...
            var threshold = 1;
            var x = t.orbitControls.mouseAction;//t.controls._activePointers;
            console.log ('x', x);
            if (x || x === 0)
                if (x===0 || x===2) {
                    //debugger;
                    ...
                    /*if (
                        //!t.controls._isUserControllingTruck
                        (
                            t.controls.deltaX
                            || t.controls.deltaY
                        )
                        && (
                            t.controls.deltaX < -1 * threshold
                            || t.controls.deltaX > threshold
                            || t.controls.deltaY < -1 * threshold
                            || t.controls.deltaY > threshold
                        )
                    ) {*/
                    ...
                    //} else {
                        ...
                        if (intersects[0] && intersects[0].object.type!=="Line") {
                            ...
                            //t.flyControls.enabled = false;
                            //if (t.debug) console.log ("pointerdown() if",t.controls.enabled, t.flyControls.enabled);
                                //t.camera.lookAt (t.s2[0].position);
                                //t.controls._camera.lookAt (t.s2[0].position);
                            ...
                            //t.controls.enabled= false;
                            //t.flyControls.enabled = true;
                            //if (t.debug) console.log ("pointerdown() else",t.controls.enabled, t.flyControls.enabled);
                            ...
                            //debugger;
                                ...
                                t.lookClock = -2;
                                t.flyControls.enabled = true;
                                t.flyControls.pointerdown (t.orbitControls.event);
                                t.flyControls.enabled = false;
                    ...
                    t.orbitControls.enabled = true;
                    t.controls.enabled =
                        t.useCameraControls && t.controlsEnabled;
                }*/
            ...
            var dbg = {
                    ...
                    //"t.controls.deltaY" : t.controls.deltaY,
                    "p" : t.camera.position,
                    "iuct" : t.controls._isUserControllingTruck,
                    "fce" : t.flyControls.enabled,
                    "cce" : t.controls.enabled,
